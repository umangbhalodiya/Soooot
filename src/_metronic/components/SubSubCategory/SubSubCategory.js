...
const SubSubCategory = ({getNewCount, title}) => {
    ...
    const getAllUpdate = async () => {
        ...
        if (!search) {
            await ApiGet(`sub_sub_category`).then((res) => {
                console.log("category22", res ?. data ?. payload ?. findSubSubCategory ?. map((data) => {
                    return {
            ...
            await ApiGet(`sub_sub_category`).then((res) => {
                console.log("category11", res ?. data ?. payload ?. findSubSubCategory);
                setIsLoaderVisible(false);
                setFilteredInterest(res ?. data ?. payload ?. findSubSubCategory);
                setCount(res ?. data ?. data ?. length);
                setCategory_id(res ?. data ?. payload ?. findSubSubCategory ?. map((data) => {
                    return {
                        value: data ?. categoryId ?. _id,
                        label: data ?. categoryId ?. name
                    };
    ...
    const handleAddInterest = (e) => {
        ...
        if (validateFormForAddAdmin()) {
            ...
            ApiPost(`sub_sub_category/create`, data).then((res) => {
                if (res ?. status == 200) {
                    setIsAddInterest(false);
    ...
    useEffect(() => {
      ApiGet(`sub_category?cid=${category_id}`).then((res) => {
          if (res ?. status == 200) {
            
              setSubCategoryList(res ?. data ?. payload ?. findSubCategory ?. map((data) => {
                  return {
                      value: data ?. _id,
                      label: data ?. name
                  };
    ...
    const handleDeleteAnnouncement = () => {
        ApiDelete(`sub_sub_category/delete?id=${idForDeleteInterest}`).then((res) => {
            if (res ?. status == 200) {
                setShow(false);
    ...
    const handleUpdateInterest = (e) => {
        ...
        if (validateForm()) {
            ...
            // }
            ApiPut(`sub_sub_category/update?id=${idForUpdateInterest}`, data).then((res) => {
                if (res ?. status == 200) {
                    setInputValueForAdd({});
                    setIsAddInterest(false);
                    setIsEditApi(false);
    ...
    const columns = [
        ...
        },
        {
          ...
          cell: (row) => {
              return <>{
                  !row ?. subCategoryId ?. name ? "-" : row ?. subCategoryId ?. name
              }</>;
          },
          selector: "name",
          sortable: true
                            ...
                            <div className="cursor-pointer pl-2"
                                onClick={
                                    () => {
                                        ...
                                        setSubCategory_id(row?.subCategoryId?._id)
                                    }
                            }>
                                <Tooltip title="Edit Product Category" arrow>
                                    <CreateIcon/>
                                </Tooltip>
                            </div>
                        </div>
    ...
    const handleDecline = async () => {
        ...
        await ApiDelete(`sub_sub_category/delete?id=${sId}&isActive=false`).then((res) => {
          if (res ?. status == 200) {
            console.log("res.data", res.data);
            getAllUpdate();
            toast.success("Product category de-activated successfully");
            handleClose();
          } else {
            getAllUpdate();
            handleClose();
    ...
    const handleApprove = async (e) => {
        await ApiDelete(`sub_sub_category/delete?id=${sId}&isActive=true`).then((res) => {
          if (res ?. status == 200) {
            console.log("res.data", res.data);
            getAllUpdate();
            handleClose();
                ...
                <div className="p-2 mb-2">
                    <div className="row mb-4 pr-3">
                        <div className="col d-flex justify-content-between">
                            <h2 className="pl-3 pt-2">Product Category
                            </h2>
                        </div>
    ...
    isAddInterest ? (
        <Dialog fullScreen
            ...
            <List> {
                ...
                {category_id ? (<div className="form-group row">
                    <label className="col-xl-3 col-lg-3 col-form-label" for="subCategory_id">
                        Choose Sub-Category
                    </label>
                    <div className="col-lg-9 col-xl-6">
                        <select className={`form-control form-control-lg form-control-solid `}
                            ...
                            <option value="" disabled selected hidden>Select</option>
                            {
                            subCategoryList ?. length > 0 && subCategoryList ?. map((item, index) => { // console.log("qqq", item);
                                return (
                                    <option key={index}
                                        value={
                                            item ?. value
    ...
    <Modal show={approve}
        ...
        <Modal.Body>
            <h6>Are you sure to want to activate this product category ?</h6>
        </Modal.Body>
        <Modal.Footer>
            <Button variant="secondary"
                onClick={handleClose}>
    ...
    <Modal show={decline}
        ...
        <Modal.Body>
            <h6>Are you sure to want to deactivate this product category ?</h6>
        </Modal.Body>
        <Modal.Footer>
            <Button variant="secondary"
                onClick={handleClose}>
